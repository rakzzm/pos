...
export default function OrdersPage() {
  ...
  const handleAddItem = (productId: string, isNew: boolean = false) => {
    ...
    if (isNew) {
      ...
      setEditedItems(prev => updateItems(prev));
    }
  };
  ...
  const handleUpdateQuantity = (productId: string, quantity: number, isNew: boolean = false) => {
    ...
    const updateItems = (items: OrderItem[]) => items.map(item =>
  ...
  const filteredOrders = orders.filter(order => {
    ...
    const matchesSource = sourceFilter === 'all' || (order.source || 'POS') === sourceFilter;
